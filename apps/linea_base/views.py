...
def lineabasejson(request):
    linea_base = LineaBase.objects.values('nombre').annotate(ucount=Count('id'))

    data = []
    for linea in linea_base:
        name = linea['nombre'] or 'Unassigned'
        r = lambda: random.randint(0, 255)
        data.append({'value': linea['ucount'], 'label': name, 'color': '#%02X%02X%02X' % (r(), r(), r())})

    return HttpResponse(json.dumps(data), content_type='application/json; utf-8')


#http://blog.algargar.com/2015/08/25/agregando-graficas-a-django-admin-con-chartjs-y-django-suit/